...
export const updatePembelianStok = async(req, res) => {
    try {
        const updatedPembelianStok = await PembelianStok.updateOne({ _id: req.params.id }, { $set: req.body });
...
export const deletePembelianStok = async(req, res) => {
    try {
        const deletedPembelianStok = await PembelianStok.deleteOne({ _id: req.params.id });
        // Status 200 = Successful
